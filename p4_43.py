...
zeta = [0,0]
A_s, M_m, K_m = eigenAnalysis(M, K, zeta)
...
v_2_prime = ((1+restitution)*v_1) / (1 + m2/float(m1))

x_0 = mat([[0],[0]])
Dx_0 = mat([[v_2_prime], [0]])
print 'Phys Pos ICs:\n' + str(x_0) +'\n'
print 'Phys Vel ICs:\n' + str(Dx_0) +'\n'			
...
if freq1 < 1e-5:
	...
	qp1 = lambda t: Qf1[0,0]/K_m[0,0]*(t**2 - 4*zeta[0]*t/freq1 - 2/K_m[0,0]*(1-4*zeta[0]**2)) + Qf1[0,1]/K_m[0,0]*(t-2*zeta[0]/freq1) + Qf1[0,2]/K_m[0,0]
	Dqp1 = lambda t: Qf1[0,0]/K_m[0,0]*(2*t - 4*zeta[0]/freq1) + Qf1[0,1]/K_m[0,0]

qp2 = lambda t: Qf2[0,0]/K_m[1,1]*(t**2 - 4*zeta[1]*t/freq2 - 2/K_m[1,1]*(1-4*zeta[1]**2)) + Qf2[0,1]/K_m[1,1]*(t-2*zeta[1]/freq2) + Qf2[0,2]/K_m[1,1]
Dqp2 = lambda t: Qf2[0,0]/K_m[1,1]*(2*t - 4*zeta[1]/freq2) + Qf2[0,1]/K_m[1,1]
...
x_1_ss = lambda t: x_unity[0,0] + t * x_lin[0,0] + t ** 2 * x_quad[0,0]
x_2_ss = lambda t: x_unity[1,0] + t * x_lin[1,0] + t ** 2 * x_quad[1,0]
...
for timestep in time:
	x1_now = x1(timestep) + x_1_ss(timestep)
	Pos1.append(x1_now)

	x2_now = x2(timestep) + x_2_ss(timestep)
	Pos2.append(x2_now)
